...
void set_supracell(int* n_max, int* m_max, int* l_max, gemmi::SmallStructure structure, double cutoff) {
  ...
  double large_cutoff = cutoff + cell_diag_halved;
  ...
  // https://math.stackexchange.com/questions/3457252/parallelepiped-size-with-sphere-inscribed
  *n_max = int(abs(large_cutoff * sqrt((b_y*c_x-b_x*c_y)*(b_y*c_x-b_x*c_y)
  ...
  *m_max = int(abs(large_cutoff * sqrt((c_y*a_x-c_x*a_y)*(c_y*a_x-c_x*a_y)
  ...
  *l_max = int(abs(large_cutoff * sqrt((a_y*b_x-a_x*b_y)*(a_y*b_x-a_x*b_y)
